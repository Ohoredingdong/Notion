...
CNN={**UA,'Origin':'https://www.cnn.com','Referer':'https://www.cnn.com/'}
...
def crypto():
    ...
    if not hist:raise RuntimeError('crypto data unavailable')
...
def korea():
    ...
    kp,qp,fp,mp=pos(k),pos(q),pos(fx,invert=True),mom(k)
    score=(kp+qp+fp+mp)/4
    hist=[]
    start=max(20,len(k)-30)
...
if __name__=='__main__':
    ...
    write('crypto-fear-greed-data.json',crypto())
    write('korea-market-sentiment.json',korea())
    print('updated US / Korea / Crypto sentiment data')
