...
const IMG={
 '두부김치':'assets/use-me-first/dubu-kimchi.webp?v=5',
 '김치볶음밥':'assets/use-me-first/kimchi-fried-rice.webp?v=5',
 '두부 김치덮밥':'assets/use-me-first/dubu-kimchi-rice.webp?v=5',
 '닭가슴살 양파 간장볶음':'assets/use-me-first/chicken-onion-soy-stirfry.webp?v=5',
 '계란찜':'assets/use-me-first/gyeran-jjim-ai.webp?v=2',
 '시금치나물':'assets/use-me-first/sigeumchi-namul-ai.webp?v=2',
 '두부부침':'assets/use-me-first/dubu-buchim-ai.webp?v=2',
 '김치두부찌개':'assets/use-me-first/kimchi-dubu-jjigae-ai.webp?v=2'
};
const C=[
...
function render(){
 ...
 el.innerHTML=`<div class="umf-menu-photo"><span class="umf-menu-num">추천 ${i+1}</span><img src="${src}" alt="${esc(r.name)}" loading="eager" decoding="async"><div class="umf-menu-fallback" style="display:none">이미지 불러오기 실패<br>${esc(r.name)}</div>${r.missing.length?`<div class="umf-menu-missing">필요: ${esc(r.missing.slice(0,2).join(' · '))}</div>`:''}</div><div class="umf-menu-body"><div class="umf-menu-name">${esc(r.name)}</div><div class="umf-menu-desc">${esc(r.desc)}</div><div class="umf-menu-meta"><span>${esc(r.time)}</span><span>${esc(r.level)}</span></div></div>`;
