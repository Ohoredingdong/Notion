(()=>{
  ...
  function check(r,st){
    const missing=[];
    r.f.forEach(x=>{if(!hasName(st.items,x))missing.push(x)});
    r.s.forEach(x=>{if(!st.season.has(x))missing.push(x)});
    r.p.forEach(x=>{if(!st.pantry.has(x))missing.push(x)});
    return missing;
  }
  ...
  function ensureShell(){
    ...
    const legacy=sec.querySelector('.recipe'); sec.insertBefore(shell,legacy);
  ...
  function renderSelection(r,st){
    selected=r;
    document.querySelectorAll('.umf-menu-card').forEach(x=>x.classList.toggle('selected',x.dataset.name===r.name));
    const chips=document.getElementById('umfSelectionChips');
    const use=actualUse(r,st);
    chips.innerHTML=use.length?use.map(x=>`<span class="umf-selection-chip">${escape(x)}</span>`).join(''):'<span class="umf-selection-chip">냉장고 재료 확인</span>';
  ...
  function render(){
    ...
    grid.innerHTML=list.map((r,i)=>{
      ...
      return `<article class="umf-menu-card${r.missing.length?' missing':''}" data-name="${escape(r.name)}"><div class="umf-menu-photo"><span class="umf-menu-num">${i+1}</span>${src?`<img src="${src}" alt="${escape(r.name)}">`:`<div class="umf-menu-fallback">${escape(r.name)}</div>`}${r.missing.length?`<div class="umf-menu-missing">필요: ${escape(r.missing.slice(0,2).join(' · '))}</div>`:''}</div><div class="umf-menu-body"><div class="umf-menu-name">${escape(r.name)}</div><div class="umf-menu-desc">${escape(r.desc)}</div><div class="umf-menu-meta"><span>${escape(r.time)}</span><span>${escape(r.level)}</span></div></div></article>`
    }).join('');
  ...
  const obsTarget=document.getElementById('list');
  if(obsTarget)new MutationObserver(()=>setTimeout(render,0)).observe(obsTarget,{childList:true,subtree:true});
