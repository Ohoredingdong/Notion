(()=>{
  ...
  function makeButton(name,label,icon){
    ...
    b.type='button';
    b.className=`lt-control-btn lt-control-${name}`;
  ...
  const sizeBtn=makeButton('size','크기 변경',icons.size);
  const themeBtn=makeButton('theme','다크 모드',icons.moon);
  const lockBtn=makeButton('lock','크기 잠금',icons.unlock);
  const resetBtn=makeButton('reset','UI 초기화',icons.reset);
  ...
  .lt-control-btn{position:relative;width:32px;height:32px;min-width:32px;min-height:32px;border:1px solid #e2e2e6;background:rgba(255,255,255,.96);color:#6e6e73;border-radius:50%;display:grid;place-items:center;padding:0;cursor:pointer;box-shadow:0 3px 12px rgba(0,0,0,.06);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);transition:transform .14s ease,background .14s ease,color .14s ease,border-color .14s ease,opacity .14s ease}
  .lt-control-btn:hover{transform:translateY(-1px);background:#f7f7f8;color:#111113;border-color:#d4d4d8}
  .lt-control-btn svg{width:14px;height:14px;fill:none;stroke:currentColor;stroke-width:1.65;stroke-linecap:round;stroke-linejoin:round}
  ...
  .lt-control-btn:after{content:attr(data-tip);position:absolute;right:39px;top:50%;transform:translate(4px,-50%);opacity:0;pointer-events:none;white-space:nowrap;background:rgba(28,28,30,.92);color:#fff;border-radius:7px;padding:5px 7px;font-size:9px;font-weight:600;line-height:1;transition:.12s ease}
  ...
  html[data-lt-theme="dark"] .wide-stats,html[data-lt-theme="dark"] .subcard,html[data-lt-theme="dark"] .calendar-side,html[data-lt-theme="dark"] .syncStatusBox,html[data-lt-theme="dark"] .syncCodeBox{background:#2c2c2e!important;border-color:#3a3a3c!important;color:#f5f5f7!important}
  html[data-lt-theme="dark"] .divider,html[data-lt-theme="dark"] .statline{background:#38383a!important}
  ...
  html[data-lt-theme="dark"] .menu,html[data-lt-theme="dark"] .syncPanel{background:#242426!important;border-color:#3a3a3c!important;color:#f5f5f7!important}
  html[data-lt-theme="dark"] .lt-control-btn{background:rgba(44,44,46,.96);color:#aeaeb2;border-color:#48484a;box-shadow:0 3px 12px rgba(0,0,0,.26)}
  ...
  @media(hover:none){.lt-control-rail{opacity:.92;visibility:visible;pointer-events:auto;transform:none}.lt-control-btn:after{display:none}}
  @media(max-width:390px){body{padding-right:43px!important}.lt-control-rail{left:calc(100% + 4px);padding-left:9px;gap:6px}.lt-control-btn{width:29px;height:29px;min-width:29px;min-height:29px}.lt-control-btn svg{width:13px;height:13px}}
  `;
  document.head.appendChild(css);

  function persist(){try{localStorage.setItem(key,JSON.stringify(state))}catch{}}
  function apply(){
    const px=sizes[state.sizeIndex];
    host.style.width='100%';
    host.style.maxWidth=px+'px';
    if(getComputedStyle(root).getPropertyValue('--card-w').trim()) root.style.setProperty('--card-w',px+'px');
    root.dataset.ltTheme=state.theme;
    ...
    themeBtn.setAttribute('aria-label',themeBtn.dataset.tip);
  }
